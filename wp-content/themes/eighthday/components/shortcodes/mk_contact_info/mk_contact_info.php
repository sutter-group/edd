...
<div class="widget_contact_info mk-contactinfo-shortcode a_margin-bottom-20" <?php echo get_schema_markup('person'); ?>>
	...
	<ul>
		...
		<?php if(!empty( $email)) { ?>
			<li><i><?php Mk_SVG_Icons::get_svg_icon_by_class_name(true, 'mk-icon-envelope', 16); ?></i><span itemprop="email"><a itemprop="email" href="mailto:<?php echo antispambot( $email ); ?>"><?php echo antispambot( $email ); ?></a></span></li>
		<?php } ?>

		<?php if(!empty( $website)) { ?>
			<li><i><?php Mk_SVG_Icons::get_svg_icon_by_class_name(true, 'mk-icon-globe', 16); ?></i><span><a href="<?php echo $website; ?>" itemprop="url"><?php echo $website; ?></a></span></li>
		<?php } ?>
